...
const calculateEmployeePayroll = async (
  ...
  const manualHourlyRate = currentUser.otHourlyRate
    ? parseFloat(currentUser.otHourlyRate)
    : null;
  ...
  let hourlyRate =
    manualHourlyRate ||
    (salaryType === "Daily" ? baseSalary / 8 : baseSalary / standardHours);

  records.forEach((rec) => {
    if (
      rec.status === "Present" ||
      rec.status === "Late" ||
      (rec.workHours && parseFloat(rec.workHours) > 0)
    ) {
      workedDaysCount++;
    }
    if (rec.workHours) totalWorkHours += parseFloat(rec.workHours);

    const otMinutes = rec.calculatedOvertimeMinutes
      ? parseFloat(rec.calculatedOvertimeMinutes)
      : rec.overtimeHours
        ? parseFloat(rec.overtimeHours) * 60
        : 0;

    if (otMinutes > 0) {
      const otHours = otMinutes / 60;
      totalOvertimeHours += otHours;
      const multiplier = rec.isHoliday
        ? holidayMult
        : rec.isWeekend
          ? weekendMult
          : weekdayMult;
  ...
  let grossBase =
    salaryType === "Daily" ? workedDaysCount * baseSalary : baseSalary;
  ...
  lineItems.push({
    componentName: "EPF Deduction (8%)",
    type: "StatutoryDeduction",
    amount: employeeEpf,
  });
  lineItems.push({
    componentName: "Employer EPF (12%)",
    type: "EmployerContribution",
    amount: employerEpf,
  });
  lineItems.push({
    componentName: "Employer ETF (3%)",
    type: "EmployerContribution",
    amount: employerEtf,
  });
  ...
  if (
    ...
    throw new Error(
      `Cannot regenerate payroll. Current status is ${existing.status}`,
    );
  }
...
export const generatePayroll = async (req: Request, res: Response) => {
  try {
    ...
    const data = await calculateEmployeePayroll(
      userId,
      month,
      year,
      generatedBy,
...
export const bulkSubmitApproval = async (req: Request, res: Response) => {
  try {
    ...
    const updatedArray = await db
      ...
      .where(and(inArray(payroll.id, payrollIds), eq(payroll.status, "Draft")))
      .returning();
...
export const bulkApprovePayroll = async (req: Request, res: Response) => {
  try {
    const { payrollIds } = req.body;
    ...
    const updatedArray = await db
      ...
      .where(
        and(
          ...
          eq(payroll.status, "Pending Approval"),
        ),
      )
      .returning();

    res.status(200).json({
      message: `Successfully approved ${updatedArray.length} record(s).`,
      count: updatedArray.length,
    });
  } catch (error: any) {
...
const bookPayrollJournal = async (payrollRecord: any, approvedBy: string) => {
  const items = await db
    .select()
    .from(payroll_item)
    .where(eq(payroll_item.payrollId, payrollRecord.id));

  // Find relevant Chart of Accounts
  const allAccounts = await db.select().from(chart_of_accounts);
  let salaryExpAccount = allAccounts.find(
    (a) =>
      a.name.toLowerCase().includes("salary ext") ||
      a.name.toLowerCase().includes("expense") ||
      a.code.startsWith("6"),
  );
  let salaryPayAccount = allAccounts.find(
    (a) =>
      a.name.toLowerCase().includes("payable") ||
      a.name.toLowerCase().includes("liability") ||
      a.code.startsWith("2"),
  );
...
export const deletePayroll = async (req: Request, res: Response) => {
  try {
    ...
    if (["Approved", "Paid"].includes(existing.status || "")) {
      return res
        .status(400)
        .json({ message: "Cannot delete processed payrolls" });
    }
